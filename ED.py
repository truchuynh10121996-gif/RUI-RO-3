...
# Load dữ liệu huấn luyện (CSV có default,
# X_1..X_14)
try:
...
# =======================================================
# PHẦN CHỈNH SỬA: ẨN BẢNG THỐNG KÊ MẶC ĐỊNH VÀ THÊM CHECKBOX
# =======================================================

# Lưu kết quả describe() vào biến (dù không hiển thị)
df_describe = df[[f"X_{i}" for i in range(1, 15)]].describe()

# Thêm Checkbox để kiểm soát hiển thị
if st.checkbox("Hiển thị thống kê mô tả dữ liệu (df.describe())"):
	st.write(df_describe)

# =======================================================
# TRAIN MODEL VẪN CHẠY BÌNH THƯỜNG VỚI DF
# =======================================================
# Train model
...
type=["xlsx"], key="ho_so_dn")
	if up_xlsx is not None:
		...
		if set(X.columns) == set(ratios_df.columns):
			with st.expander("Xác suất vỡ nợ dự báo (nếu đã huấn luyện ở
trên)"):
				try:
					probs =
model.predict_proba(ratios_df[X.columns])[:, 1]
					preds = (probs >=
0.5).astype(int)
					show = ratios_df.copy()
					show["pd"] =
...
báo được PD: {e}")
		...
		# Gemini Phân tích & khuyến nghị - ĐOẠN CODE BẠN YÊU CẦU THÊM VÀO
# ĐÂY
