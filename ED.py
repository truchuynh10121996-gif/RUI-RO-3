...
# Thư viện GOOGLE GEMINI VÀ OPENAI (Giữ nguyên logic kiểm
# tra thư viện)
try:
...
/*
------------------------------------------------------------------------------------------------
| THAY ĐỔI 1: Dải Banner Cho Tiêu đề Chính (Bao gồm cả Tiêu
| đề và Subtitle)
| Đã đảm bảo container này sẽ chiếm toàn bộ chiều rộng (nhờ
| layout="wide" và không có max-width)
------------------------------------------------------------------------------------------------
*/
.banner-title-container {
    background: linear-gradient(90deg, #e0f0ff, #f7f9fc, #e0f0ff); /* Màu chuyển sắc nhẹ nhàng */
    padding: 20px 30px; /* Tăng padding để làm
    dải banner dày hơn */
    border-radius: 10px; /* Bo góc nhẹ */
    box-shadow: 0 4px 12px rgba(0, 76, 153, 0.1); /* Shadow nhẹ nhàng, chuyên nghiệp */
    margin-bottom: 20px; /* Khoảng cách với nội
    dung bên dưới */
    text-align: center;
    /* Streamlit mặc định có một lớp wrapper,
    lớp này sẽ đảm bảo container này rộng hết mức trong layout="wide" */
}
/* Đảm bảo h1 trong banner sử dụng màu sắc đồng bộ */
.banner-title-container h1 {
    color: #004c99 !important; /* Xanh Navy Đậm
    cho tiêu đề */
    font-weight: 900 !important;
    text-shadow: 1px 1px 3px rgba(0,0,0,0.1);
    animation: none; /* Tắt animation 'wave' của
    h1 nếu nó quá nổi bật */
}
/* Đảm bảo h3 (subtitle) trong banner sử dụng màu sắc đồng
bộ */
.banner-title-container h3 {
    color: #1a75ff !important; /* Xanh tươi hơn
    cho tiêu đề phụ */
    font-weight: 600 !important;
    margin-top: -10px; /* Kéo lên gần tiêu đề
    chính */
    border-bottom: none;
}
/*
...
/* Tiêu đề chính (Header) - Màu xanh đậm chuyên nghiệp +
Hiệu ứng lượn sóng nhẹ (animation) */
h1 {
...
[data-testid="stSidebar"] {
    background-color: #e0f0ff; /* Xanh nhạt cho
    sidebar */
}
...
.stTabs [data-testid="stHorizontalBlock"] {
    background-color: #f0f8ff; /* Nền nhẹ cho
    thanh tab */
    border-radius: 8px;
    padding: 5px 0;
}
/* Cải thiện khả năng nhìn của tab bar */
.stTabs [role="tablist"] {
    gap: 10px; /* Khoảng cách giữa các tab */
    border-bottom: 2px solid #004c99; /* Đường
    viền dưới đậm hơn */
}
.stTabs button {
    font-weight: bold;
...
def get_ai_analysis(data_payload: dict, api_key: str) -> str:
    """
    Sử dụng Gemini API để phân tích chỉ số tài
    chính.
    """
    if not _GEMINI_OK:
        return "Lỗi: Thiếu thư
        viện google-genai (cần cài đặt: pip install google-genai)."
 
    client = genai.Client(api_key=api_key)
 
    sys_prompt = (
        "Bạn là chuyên gia phân
        tích tín dụng doanh nghiệp tại ngân hàng. "
        "Phân tích toàn diện dựa
        trên 14 chỉ số tài chính được cung cấp và PD nếu có. "
        "Nêu rõ: (1) Khả năng sinh
        lời, (2) Thanh khoản, (3) Cơ cấu nợ, (4) Hiệu quả hoạt động. "
        "Kết thúc bằng khuyến nghị
        in hoa: CHO VAY hoặc KHÔNG CHO VAY, kèm 2–3 điều kiện nếu CHO VAY. "
        "Viết bằng tiếng Việt súc
        tích, chuyên nghiệp."
    )
    
    # Gửi tên tiếng Việt dễ hiểu hơn cho AI
    user_prompt = "Bộ chỉ số tài chính và PD
...
# =========================
# TÍNH X1..X14 TỪ 3 SHEET (CDKT/BCTN/LCTT) - SỬ DỤNG TÊN
...
def _pick_year_cols(df: pd.DataFrame):
    """Chọn 2 cột năm gần nhất từ
    sheet (ưu tiên cột có nhãn là năm)."""
    numeric_years = []
...
def _get_row_vals(df: pd.DataFrame, aliases: list[str]):
    """Tìm dòng theo alias. Trả về
    (prev, cur) theo 2 cột năm gần nhất."""
...
def compute_ratios_from_three_sheets(xlsx_file) -> pd.DataFrame:
    """Đọc 3 sheet CDKT/BCTN/LCTT
    và tính X1..X14 theo yêu cầu."""
    ...
    # ---- Tính toán các biến số tài chính (GIỮ
    # NGUYÊN CÁCH TÍNH)
...
#
# ------------------------------------------------------------------------------------------------
# THAY ĐỔI 1: Áp dụng dải banner CSS đã tạo (banner rộng
# hơn)
# ------------------------------------------------------------------------------------------------
...
# Hiển thị trạng thái thư viện AI (Sử dụng cột để bố trí
# đẹp hơn)
...
# Load dữ liệu huấn luyện (CSV có default, X_1..X_14) - Giữ
# nguyên logic load data
try:
    df = pd.read_csv('DATASET.csv', encoding='latin-1')
    # Tên cột cho việc huấn luyện (phải giữ
    # nguyên X_1..X_14)
...
#
# ------------------------------------------------------------------------------------------------
# THAY ĐỔI 2: GIỮ NGUYÊN st.tabs NGANG (Đã đáp ứng yêu
# cầu chuyển sang tab ngang)
# ------------------------------------------------------------------------------------------------
...
with tab_build:
    ...
    # Biểu đồ Scatter Plot và Đường Hồi quy
    # Logisitc (GIỮ NGUYÊN LOGIC)
    if col in df.columns:
        try:
            # Dùng
            # Streamlit.pyplot để đảm bảo tích hợp tốt hơn
            fig, ax = plt.subplots(figsize=(10, 6))
            sns.scatterplot(data=df, x=col, y='default', alpha=0.6, ax=ax, hue='default', palette=['#1a75ff', '#ff4b4b']) # Dùng màu sắc theme
            
            # Vẽ đường
            # logistic regression theo 1 biến
    ...
    with col_cm:
        ...
        # Sử dụng cmap màu xanh đậm hơn
        # để đồng bộ với theme
...
with tab_predict:
    ...
    # Sử dụng st.container và st.expander để tổ
    # chức khu vực upload
    ...
    if up_xlsx is not None:
        # Tính X1..X14 từ 3 sheet (GIỮ
        # NGUYÊN)
        try:
            # Hiển thị thanh
            # tiến trình giả lập (thêm hiệu ứng động)
            with st.spinner('Đang đọc và xử lý dữ liệu tài chính...'):
                ratios_df = compute_ratios_from_three_sheets(up_xlsx)
            
            # Tách riêng 14
            # cột tiếng Việt (hiển thị) và 14 cột tiếng Anh (dự báo)
            # ratios_display
            # là DataFrame 1 cột: Index (Tên chỉ số) | Giá trị
        ...
        # Tạo payload data cho AI (Sử
        # dụng tên tiếng Việt)
        data_for_ai = ratios_display.to_dict()['Giá trị']
        
        # (Tuỳ chọn) dự báo PD nếu mô
        # hình đã huấn luyện đúng cấu trúc X_1..X_14
        probs = np.nan
        preds = np.nan
        # Kiểm tra mô hình có sẵn sàng
        # dự báo không (đã train và cột khớp)
        ...
        # PYTHON: Bỏ .T để hiển thị đúng Tên Biến | Con số
        # ------------------------------------------------------------------------------------------------
        ...
        # ratios_display đã có cấu trúc
        # đúng: Index (Tên biến) | Giá trị (Con số)
        ...
        with pd_col_pd:
            ...
            st.metric(
                ...
                # Đảo ngược màu sắc delta cho PD: Rủi ro cao là màu đỏ (inverse), rủi ro thấp là
                # màu xanh (normal)
